...
def plot_one_box(x, im, color=(128, 128, 128), label=None, line_thickness=3):
    ...
    x[0] = x[0] - 10 if x[0] - 10 > 0 else 0
    x[1] = x[1] - 10 if x[1] - 10 > 0 else 0
    x[2] = x[2] + 10 if x[2] + 10 < im.shape[1]else im.shape[1]
    x[3] = x[3] + 20 if x[3]+20<im.shape[0] else im.shape[0]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    cv2.rectangle(im, c1, c2, color, thickness=tl, lineType=cv2.LINE_AA)
    print("左上点的坐标为：(" + str(c1[0]) + "," + str(c1[1]) + ")，右下点的坐标为(" + str(c2[0]) + "," + str(c2[1]) + ")")
    # box1 = (c1[0], c1[1], c2[0], c2[1])  # 设置左、上、右、下的像素
    # image = cv2_crop(im, box1)
    # cv2.imwrite('../cj/'+str(id)+'.jpg', image)
    if label:
